						...
						Bootstrapper &bootstrapper, KeyGenerator &keygen,CKKSEncoder &encoder, Encryptor &encryptor, Decryptor &decryptor, Evaluator &evaluator, GaloisKeys &gal_keys, RelinKeys &relin_keys)
{
    ...
    // Q,K,V projection
    attn_proj_row_seal(A,qw,qb,Q,128,768,768,768,keygen,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    attn_proj_row_seal(A,kw,kb,K,128,768,768,768,keygen,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    attn_proj_col_seal(A,vw,vb,V,128,768,768,768,keygen,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    ...
    qk_matmul(Q,K,QKt,128,768,768,128,keygen,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    ...
    // Apply softmax
    for(i=0;i<QKt.size();i++)
        compute_softmax(QKt[i],6,Bootstrapper &bootstrapper,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    
    vc pre_out;
    init_output(8,QKt,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
    sv_matmul(QKt,V,pre_out,128,128,128,768,keygen,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
