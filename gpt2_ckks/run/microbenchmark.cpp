...
void benchmark_softmax(SEALContext context,KeyGenerator &keygen,Bootstrapper &bootstrapper, CKKSEncoder &encoder, Encryptor &encryptor, Decryptor &decryptor,Evaluator &evaluator,GaloisKeys &gal_keys, RelinKeys &relin_keys) {
...
int main(int argc, char* argv[]) {
	...
	int remaining_level = 18; // Calculation required
    ...
    switch(entry) {
        ...
        case SOFTMAX:
            cout <<"Executing: " << tests[entry].name <<endl;
            benchmark_softmax(context,keygen,bootstrapper,encoder,encryptor,decryptor,evaluator,gal_keys,relin_keys);
            cout << "Done Executing " << tests[entry].name << endl;
            break;
        default:
